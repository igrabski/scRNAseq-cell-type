...
library(Seurat)
source('cell_type_identification3_clean.R')
```
...
As an example, we show below how we can find the barcodes for each cell-type, then identify genes with a high probability of being on in NK cells, but a low probability of being on in the others (i.e. potential markers for this cell-type):
...
barcodes[which(barcodes[,'CD4']<0.05&barcodes[,'CD14']<0.05&
                 barcodes[,'CD8']<0.05&barcodes[,'NK']>0.95),]
```
